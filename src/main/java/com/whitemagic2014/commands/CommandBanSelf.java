...
/**
 * 此类由 Hykilpikonna 在 2018/07/11 创建! Created by Hykilpikonna on 2018/07/11!
 * Github: https://github.com/hykilpikonna QQ: admin@moecraft.cc -OR- 871674895
 *
 * @author Hykilpikonna
 */
public class CommandBanSelf extends BaseCommand implements GroupCommand {
	@Override
	...
	public String groupMessage(EventGroupMessage event, GroupUser sender, Group group, String command,
		...
		try {
			if (args.size() == 0)
				...
				period = Long.parseLong(args.get(0));
		...
		return "为什么要作死呢? @" + getGroupNameNoCache(event,sender);
	}
}
