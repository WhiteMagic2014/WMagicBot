...
/**
 * 此类由 Hykilpikonna 在 2018/05/26 创建! Created by Hykilpikonna on 2018/05/26!
...
public class CommandVersion implements EverywhereCommand // 继承EverywhereCommand就是无论私聊还是群聊都能收到的指令
{
	...
	public String run(EventMessage event, User sender, String command, ArrayList<String> args) {
		...
		// 返回版本信息
		return new MessageBuilder().add("PicqBotX v").add(VERSION).newLine().add("- 酷Q版本: ")
				.add(capitalizeFirstLetterOfEachWord(versionInfo.getCoolqEdition())).newLine().add("- 插件版本: ")
				.add(versionInfo.getPluginVersion()).add(" Build ").add(versionInfo.getPluginBuildNumber()).newLine()
				.add("- 运行状态: ").add(status.getGood() ? "Good" : "Bad").toString();
	}
}
